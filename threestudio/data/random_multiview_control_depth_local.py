...
from threestudio.data.dataset_objaverse_sketch_np import Warper, sketch_warp_new, sketch_warp_new_edit
...
class RandomMultiviewCameraIterableDataset(RandomCameraIterableDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        mask_img = cv2.erode(mask_img, kernel,iterations = 1).reshape(256,256,1)  # reshape(1, 1, 256,256)
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # Change the scale of sketch based on Fov
        sketch_scale = torch.tan(0.5 * fovy[0]) / torch.tan(0.5 * self.sketch_fovy)
        sketch_grid = self.sketch_grid * sketch_scale
        
        mask_img = self.mask_img.numpy()
        depth = self.depth.reshape(256,256)
        
        source_warp = sketch_warp_new_edit(self.warper, mask_img, self.source[0], depth, \
            np.deg2rad(self.cfg.eval_elevation_deg), np.deg2rad(elevation_deg[1].item()), np.deg2rad(self.cfg.azimuth_input), np.deg2rad(azimuth_deg[1].item()), 
            radius=self.cfg.eval_camera_distance)
        
        source = self.source * (mask_img > 125.0)
        source = torch.tensor(source)
        source = einops.rearrange(source, 'f h w c -> f c h w').clone()
        source_warp = torch.tensor(source_warp)
        source_warp = source_warp.reshape(1,1,256,256)
        warp_input = torch.cat([source, source_warp, self.mask], dim=1)
        warp_output = F.grid_sample(warp_input, sketch_grid, mode='bilinear', padding_mode='zeros', align_corners=False)
        warp_output_np = warp_output.numpy()
        
        # Warp Sketch
        source_list = []
        for i in range(5):
            if i == 0:
                source_list.append(warp_output_np[:,0:1,:,:].reshape(256,256,1))
            elif i == 1:
                source_list.append(warp_output_np[:,1:2,:,:].reshape(256,256,1))
            else:
...
@register("random-multiview-control-depth-local")
class RandomMultiviewCameraDataModule(pl.LightningDataModule):
    ...
    #         # very important to disable multi-processing if you want to change self attributes at runtime!
    #         # (for example setting self.width and self.height in update_step)
    #         # num_workers=8, # num_workers=16,  # type: ignore
    #         num_workers=32,
    #         batch_size=None,
