...
from torch.multiprocessing import Manager, Lock, Value, Array
# 创建Manager()并共享变量
manager = Manager()
height_proxy = manager.Value('i', 256)  # 初始分辨率为128
width_proxy = manager.Value('i', 256)  # 初始分辨率为128
share_directions_unit_focal = manager.list(torch.zeros(10))
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        ...
        self.height = height_proxy
        self.width = width_proxy
        self.height.value = self.heights[0]
        self.width.value = self.widths[0]
        self.batch_size: int = self.batch_sizes[0]
        # self.directions_unit_focal = self.directions_unit_focals[0]
        self.directions_unit_focal = share_directions_unit_focal
        self.directions_unit_focal[0] = self.directions_unit_focals[0]
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        with lock:
            self.height.value = self.heights[size_ind]
            self.width.value = self.widths[size_ind]
            self.directions_unit_focal[0] = self.directions_unit_focals[size_ind]
