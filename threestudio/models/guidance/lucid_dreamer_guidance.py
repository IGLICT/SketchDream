...
from threestudio.models.prompt_processors.lucid_dreamer_processor import LucidDreamerPromptProcessor
...
@threestudio.register("lucid-dreamer-guidance")
class LucidDreamerGuidance(BaseModule):
    @dataclass
    class Config(BaseModule.Config):
        lambda_guidance: float = 0.1
        
        # Set Stable Diffusion and LucidDreamer hyperparameter
        ...
        # LucidDreamer debug path
        save_folder: str = './debug/lucid_output/'
        ...
        sds: bool = False
        negative_w: float = -2.
        front_decay_factor: float = 2.
        side_decay_factor: float = 10.  
    ...
    def configure(self) -> None:
        threestudio.info(f"[INFO] Loading stable diffusion for LucidDreamer ...")

        self.precision_t = torch.float16 if self.cfg.fp16 else torch.float32

        model_key = self.cfg.model_key
        assert model_key is not None

        is_safe_tensor = self.cfg.is_safe_tensor
        base_model_key = "stabilityai/stable-diffusion-v1-5" if self.cfg.base_model_key is None else self.cfg.base_model_key # for finetuned model only
        ...
        if self.cfg.use_control_net:
            controlnet_model_key = self.cfg.controlnet_model_key
            self.controlnet_depth = ControlNetModel.from_pretrained(controlnet_model_key,torch_dtype=self.precision_t).to(self.device)
        ...
        if self.cfg.textual_inversion_path is not None:
            pipe.load_textual_inversion(self.cfg.textual_inversion_path)
            print("load textual inversion in:.{}".format(self.cfg.textual_inversion_path))
        ...
        self.num_train_timesteps = self.cfg.num_train_timesteps if self.cfg.num_train_timesteps is not None else self.scheduler.config.num_train_timesteps        
        ...
        self.warmup_step = int(self.num_train_timesteps*(self.cfg.max_t_range-self.cfg.max_step_percent))

        self.noise_temp = None
        self.noise_gen = torch.Generator(self.device)
        self.noise_gen.manual_seed(self.cfg.noise_seed)
    ...
    def train_step(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
                    grad_scale=1, use_control_net=False, controlnet_cond=None, 
                    save_folder:Path=None, iteration=0, warm_up_rate = 0,
                    resolution=(512, 512), guidance_opt=None,as_latent=False, embedding_inverse = None):

        if use_control_net:
            pred_rgb, pred_depth, pred_alpha, controlnet_cond = self.augmentation(pred_rgb, pred_depth, pred_alpha, controlnet_cond)
        else:
            pred_rgb, pred_depth, pred_alpha = self.augmentation(pred_rgb, pred_depth, pred_alpha)
        ...
        with torch.no_grad():
            ...
            latent_model_input = self.scheduler.scale_model_input(latent_model_input, tt[0])
            if use_control_net:
                controlnet_cond = controlnet_cond[None, :, ...].repeat(2, 1, 1, 1, 1).reshape(-1, 3, 512, 512).half()
                down_block_res_samples, mid_block_res_sample = self.controlnet_depth(
                    latent_model_input,
                    tt,
                    encoder_hidden_states=text_embeddings,
    ...
    def __call__(
        ...
        pred_depth: Float[Tensor, "B H W C"],
        pred_alpha: Float[Tensor, "B H W C"],
        prompt_utils: LucidDreamerPromptProcessor,
        global_step: int, 
        ...
        **kwargs,
        ...
        if self.cfg.perpneg:
            text_z, weights, text_z_inverse = prompt_utils.get_text_embeddings_perpneg(elevation, azimuth)
            out_image = rgb.permute(0,3,1,2)
            out_image = F.interpolate(out_image, (512, 512), mode="bilinear", align_corners=False)
            pred_depth = pred_depth.permute(0,3,1,2)
            pred_depth = F.interpolate(pred_depth, (512, 512), mode="bilinear", align_corners=False)
            pred_alpha = pred_alpha.permute(0,3,1,2)
            pred_alpha = F.interpolate(pred_alpha, (512, 512), mode="bilinear", align_corners=False)
            loss_lucid = self.train_step_perpneg(text_z, out_image, 
                                                # pred_depth=None, pred_alpha=None,
                                                pred_depth=pred_depth, pred_alpha=pred_alpha,
                                                grad_scale=self.cfg.lambda_guidance,
                                                use_control_net = False, save_folder = self.cfg.save_folder, iteration = global_step, warm_up_rate=0., 
                                                weights = weights, resolution=(512, 512),
                                                guidance_opt=self.cfg, as_latent=False, embedding_inverse = text_z_inverse)
        else:
            text_z, text_z_inverse = prompt_utils.get_text_embeddings(elevation, azimuth)
            out_image = rgb.permute(0,3,1,2)
            out_image = F.interpolate(out_image, (512, 512), mode="bilinear", align_corners=False)
            pred_depth = pred_depth.permute(0,3,1,2)
            pred_depth = F.interpolate(pred_depth, (512, 512), mode="bilinear", align_corners=False)
            pred_alpha = pred_alpha.permute(0,3,1,2)
            pred_alpha = F.interpolate(pred_alpha, (512, 512), mode="bilinear", align_corners=False)
            loss_lucid = self.train_step(text_z, out_image, 
                                        # pred_depth=None, pred_alpha=None,
                                        pred_depth=pred_depth, pred_alpha=pred_alpha,
                                        grad_scale=self.cfg.lambda_guidance,
                                        use_control_net = False ,save_folder = self.cfg.save_folder,  iteration = global_step, warm_up_rate=0., 
                                        resolution=(512, 512),
                                        guidance_opt=self.cfg, as_latent=False, embedding_inverse = text_z_inverse)
            loss_lucid = loss_lucid[0]

        return {
            "loss_lucid": loss_lucid
        }
